...
const char * llama_model_type_name(e_model type) {
    switch (type) {
        ...
        case MODEL_125B_A6B:      return "125B.A6B";
...
bool llama_model_supports_ctx_shift(const struct llama_model * model) {
    // openPangu and DeepSeek4/4.1 keep position-dependent private state outside the generic KV cache.
    return model && model->arch != LLM_ARCH_OPENPANGU && model->arch != LLM_ARCH_DEEPSEEK4 && model->arch != LLM_ARCH_DEEPSEEK41;
}
