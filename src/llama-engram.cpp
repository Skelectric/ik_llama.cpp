...
bool llama_engram_prepare_inputs(llama_context & lctx, const llama_batch & ubatch) {
    ...
    // Image spans arrive as pure-embedding ubatches (mtmd decode_embd_batch sets
    // tokens = nullptr): every position in such a batch is an image token. The
    // server never places the image placeholder id in the token stream, so the
    // per-token id check below only fires for hypothetical mixed batches.
    const bool all_image = ubatch.token == nullptr;
    if (all_image && ubatch.embd == nullptr) {
        return true; // neither tokens nor embeddings: nothing to do
    }
    ...
    for (uint32_t i = 0; i < n_tok; ++i) {
        const llama_token   token = all_image ? DSV41_IMAGE_TOKEN_ID : ubatch.token[i];
    ...
    if (en.gate_mask != nullptr && en.gate_mask->ne[0] == (int64_t) n_tok) {
        ...
        for (uint32_t i = 0; i < n_tok; ++i) {
            m[i] = (all_image || ubatch.token[i] == DSV41_IMAGE_TOKEN_ID) ? 0.0f : 1.0f;
        }
