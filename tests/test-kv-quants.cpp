// Phase 4 (DeepSeek-V4.1) - CPU test for the packed KV-cache storage types.
//
...
// registered type_traits - so this also checks that B.1 registered the three
// types with the right size, block size and name.
//
// The self-test covers what can be checked without the reference oracle: the
// amax floors, saturation, the reference packing order, and the rounding ties
// that were verified against the reference's own CUDA kernels
// (dsv41-test/evidence/phase4/gpu-kernel-check.txt). The emit mode is driven by
// dsv41-test/scripts/check/phase4_kv_quants_check.py, which diffs it against the
// frozen oracle corpus (dsv41-test/evidence/phase4/quant-vectors.npz).
//
...
// with (phase4_gpu_kernel_check.py --rounding-probe): each is a midpoint of two
// e2m1 / e4m3 grid values, where RNE and truncation disagree. The observed
// reference values are the RNE ones.
