...
static common_chat_params common_chat_params_init_deepseek_v3_2(const common_chat_template &    tmpl,
    ...
    // V4.1 moves the leading space into the DSML tag names: "<｜DSML｜ calls>" blocks with
    // "<｜DSML｜ invoke>" / "<｜DSML｜ parameter>" tags, where V3.2/V4 used "<｜DSML｜function_calls>"
    // / "<｜DSML｜invoke>" / "<｜DSML｜parameter>" without a space (reference: DeepSeek-V4.1-Flash
    // encoding/README.md; confirmed in llama.cpp PR #28696 issuecomment-5652157165). Detect via
    // the leading-space tag literal in the template source.
    ...
    auto parser = build_chat_peg_parser([&](common_chat_peg_builder & p) {
        ...
        if (extract_reasoning && inputs.enable_thinking) {
            ...
            // Thinking disabled but reasoning extraction requested: consume the thinking
            // delimiters the generation prompt placed. V3.2 emits an empty <think></think>
            // pair, while V4 emits a bare </think>.
            reasoning = is_v4
