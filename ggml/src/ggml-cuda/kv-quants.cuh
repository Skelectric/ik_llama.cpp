...
// Phase 4 (DeepSeek-V4.1) packed KV-cache storage types - CUDA device code.
//
// The device mirror of ggml/src/ggml-kv-quants.c: the same reference rule, the
// same rounding (ties to even), and the same packing order (element 2j in the
// low nibble, element 2j+1 in the high one). The two implementations must
// produce identical bytes - that is what the B.5 check asserts.
//
// Do NOT reuse kvalues_mxfp4 or the MXFP4 scale decode here. Those are the
// fork's MXFP4 convention (a table with the values doubled, and an e8m0 byte
// that decodes one exponent lower), not the reference's. See the plan, section
// 3.2.
//
// The read helpers have the dequantize_kernel_t signature (see
// dequantize.cuh). Note that ggml-cuda's k_get_rows writes the pair as
// (dst[iqs], dst[iqs + qk/2]) - the fork's split-half convention - while these
// types store the reference's adjacent pairs, so the helper reads the two
// bytes that hold elements iqs and iqs + qk/2 rather than one byte's nibbles.
...
// e4m3fn -> fp32. The subnormal grid is m * 2^-9, not 2^-6 * (1 + m/8): the
// implicit leading 1 of the normal form is what makes it a separate case.
...
// Written out longhand, like the CPU version, so the rounding is visible.
...
// Round |q| (already clamped to 6) onto the e2m1 grid, ties to even. A tie can
// only fall between an odd and an even mantissa, i.e. on one of 0.25, 0.75,
// 1.25, 1.75, 2.5, 3.5, 5.0, and it goes to the power-of-two neighbour.
...
// ---------------------------------------------------------------------------
// the read side (ggml_get_rows source): one (v.x, v.y) pair per call, with
// v.x the element at iqs and v.y the element at iqs + qk/2 - see the note at
