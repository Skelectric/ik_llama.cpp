...
struct bin_bcast_cuda {
    ...
    void operator()(const struct ggml_tensor * src0, const struct ggml_tensor * src1, struct ggml_tensor * dst,
...

        {
            ...
            // gridDim.y and gridDim.z are both limited to 65535; a broadcast mul whose
            // collapsed ne1 (e.g. a long-context dim) exceeds it needs the 1D fallback
            // (observed: DSV4 indexer score with n_lid = 65536 -> grid.y = 65536 ->
            // "invalid configuration argument").
            if (block_nums.y > 65535 || block_nums.z > 65535) {
                // this is the maximum number of blocks in the y/z dimensions, fallback to 1D grid kernel
