...
// Phase 4 (DeepSeek-V4.1) packed KV-cache storage types.
//
// One scale byte per block, then the codes. The e2m1 codes are packed two per
// byte in the *reference* order (element 2j in the low nibble, element 2j+1 in
// the high one - torch.float4_e2m1fn_x2), so a packed block is comparable with
// the reference's own packed output.
//
// Note that these blocks are odd-sized (9, 17 and 33 bytes), exactly like
// block_mxfp4 above (17 bytes).
