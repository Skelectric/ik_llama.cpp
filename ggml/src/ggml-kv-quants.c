...
// Phase 4 (DeepSeek-V4.1): CPU quantisers for the packed KV-cache storage
// types. See ggml-kv-quants.h for what these types are and where the rule
// comes from.
...
// ---------------------------------------------------------------------------
// bit tricks - the reference's kernel.py:25-37, reproduced bit for bit
// ---------------------------------------------------------------------------
...
// e4m3 (e4m3fn) <-> fp32
//
// Decode is a direct bit construction of the e4m3fn encoding: 1 sign bit, 4
// exponent bits (bias 7), 3 mantissa bits. There are no infinities, and NaN
// (0x7F/0xFF) is never produced by the encoder below.
//
// Encode rounds to nearest, ties to even, and saturates at +-448. It is
// written out longhand - the subnormal grid m * 2^-9 below 2^-6, then the
// 3-bit mantissa - so that the rounding rule is visible and testable rather
// than hidden in a cast.
// ---------------------------------------------------------------------------

// e4m3 subnormals are the uniform grid m * 2^-9 for m = 0..7 (the e4m3fn
// subnormal grid joins the normal one smoothly: the spacing is 2^-9 up to 2^-6)
...
static inline float kv_e4m3_to_fp32(uint8_t v) {
    ...
    if (e == 0u) {
        // subnormal: m * 2^-9, not 2^-6 * (1 + m/8) - the implicit leading 1
        // of the normal form is what makes this a separate case
...
// e2m1
//
// The code is the e2m1 encoding itself: bits 2..0 are the magnitude, bit 3 is
// the sign. The magnitude grid is {0, 0.5, 1, 1.5, 2, 3, 4, 6}.
// ---------------------------------------------------------------------------
...
// 1.25, 1.75, 2.5, 3.5, 5.0, and it goes to the power-of-two neighbour - which
// is exactly what the oracle's searchsorted + parity rule does.
